...
class DOMbinder {
    ...
    harvest() {
        ...
        const harvest_from_repeater = (el) => {
            const els_form_elements = el.querySelectorAll('select');
            ...
            for (let i = 0; i < els_inputs.length; i++) {
                els_custom_selects.push(els_inputs[i]);
            }
        ...
        for (let n in this.#state) {
            if (this.#state[n].hasOwnProperty('repeater')) {
                for (let i = 0; i < this.#state[n].elements.length; i++) {
                    harvest_from_repeater(this.#state[n].elements[i]);
                }
            }
            else {
                for (let i = 0; i < this.#state[n].elements.length; i++) {
                    register_form_element(n, this.#state[n].elements[i]);
                }
            }
        }
        return values;
