    ...
    harvest() {
        ...
        const register_form_element = (name, el) => {
            if (el.tagName === 'SELECT' || (this.#tag_synonyms.hasOwnProperty('SELECT') && this.#tag_synonyms.SELECT.includes(el.tagName))) {
                ...
                values[name].push(select_values);
            }
            else if (
        ...
        const els_selects = this.#el_conainer.querySelectorAll('select');
        ...
        if (this.#tag_synonyms.hasOwnProperty('SELECT')) {
            for(let i = 0; i < this.#tag_synonyms.SELECT.length; i++) {
                const els_custom_selects = this.#el_conainer.querySelectorAll(this.#tag_synonyms.SELECT[i]);
                for (let j = 0; j < els_custom_selects.length; j++) {
                    els_form_elements.push(els_custom_selects[j]);
                }
            }
        }
        const els_inputs = this.#el_conainer.querySelectorAll('INPUT');
        ...
        if (this.#tag_synonyms.hasOwnProperty('INPUT')) {
            for (let i = 0; i < this.#tag_synonyms.INPUT.length; i++) {
                const els_custom_inputs = this.#el_conainer.querySelectorAll(this.#tag_synonyms.INPUT[i]);
        ...
        const els_textareas = this.#el_conainer.querySelectorAll('TEXTAREA');
        for (let i = 0; i < els_textareas.length; i++) {
            els_custom_inputs.push(els_textareas[i]);
        }
        if (this.#tag_synonyms.hasOwnProperty('TEXTAREA')) {
            const els_custom_textareas = this.#el_conainer.querySelectorAll(this.#tag_synonyms.TEXTAERA[i]);
